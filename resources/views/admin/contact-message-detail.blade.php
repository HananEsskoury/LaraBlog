...
<style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* MESSAGE DETAIL */
        .message-detail { max-width: 800px; margin: 0; }
        .detail-card {
            background: #fff;
            border-radius: 16px;
        ...
        .badge-unread { background: var(--yellow-bg); color: var(--yellow); }
        ...
        /* BOUTONS ALIGNÉS À GAUCHE (comme dans l'image) */
        .action-buttons {
            margin-top: 28px;
            display: flex;
            gap: 12px;
        ...
        .btn-mark:hover { background: var(--terra-dark); transform: translateY(-1px); }
        .btn-delete {
...
{{-- ════════════════════════════════════════════
     MAIN CONTENT - DETAIL MESSAGE
════════════════════════════════════════════ --}}
<div class="admin-shell">
    <main class="main">

        <div class="page-header">
            <h1>Détail du <em>message</em></h1>
            <p>{{ $message->name }} - {{ $message->created_at->format('d/m/Y H:i') }}</p>
        </div>
        ...
        <div class="tabs">
            ...
            <a href="{{ route('admin.contact.messages') }}" class="tab active">Messages</a>
        </div>
            ...
            <div class="detail-card">
                ...
                <div class="detail-body">
                    ...
                    <div class="info-row">
                        ...
                        <div class="info-value">{{ $message->ip }}</div>
                    </div>
                    @endif
                    ...
                    <div class="action-buttons">
                        <a href="{{ route('admin.contact.messages') }}" class="btn-back">
                            ← Retour
                        </a>
                        @if(!$message->is_read)
                            <form method="POST" action="{{ route('admin.contact.message.toggle', $message->id) }}" style="display: inline;" id="form-mark-{{ $message->id }}">
                                @csrf @method('PATCH')
                                <button type="button" class="btn-mark" onclick="openConfirmMark({{ $message->id }})">
                                    ✓ Marquer comme lu
                                </button>
                            </form>
                        @endif
                        <form method="POST" action="{{ route('admin.contact.message.delete', $message->id) }}" style="display: inline;" id="form-delete-{{ $message->id }}">
                            @csrf @method('DELETE')
                            <button type="button" class="btn-delete" onclick="openConfirmDelete({{ $message->id }}, '{{ addslashes($message->name) }}')">
                                🗑️ Supprimer
                            </button>
                        </form>
                    </div>
...
<div id="confirmDeleteModal" style="display:none;position:fixed;inset:0;background:rgba(42,32,24,.55);
    ...
    <div style="background:#fff;border-radius:16px;width:420px;max-width:100%;
        ...
        <div style="padding:14px 20px;display:flex;align-items:center;gap:10px;background:var(--red-bg);">
            <span style="font-size:20px;">🗑️</span>
            <span style="font-size:11px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--red);">Suppression</span>
        </div>

        <div style="padding:24px 28px 20px;">
            <p style="font-family:var(--serif);font-size:1.35rem;font-weight:400;color:var(--ink);margin-bottom:8px;line-height:1.3;">
                Supprimer ce message ?
            </p>
            <p style="font-size:13px;color:var(--muted);font-weight:300;line-height:1.6;">
                Vous êtes sur le point de supprimer le message de <strong id="deleteMessageName"></strong>. 
        ...
        <div style="padding:0 28px 24px;display:flex;justify-content:flex-end;gap:10px;">
            ...
            <button id="confirmDeleteBtn"
                style="padding:9px 22px;border-radius:8px;border:none;background:var(--red);
...
{{-- ── MODALE DE CONFIRMATION MARQUER COMME LU ── --}}
<div id="confirmMarkModal" style="display:none;position:fixed;inset:0;background:rgba(42,32,24,.55);
     backdrop-filter:blur(4px);z-index:5000;align-items:center;justify-content:center;padding:24px;">
    <div style="background:#fff;border-radius:16px;width:420px;max-width:100%;
                box-shadow:0 24px 80px rgba(0,0,0,.25);overflow:hidden;
                animation:modalIn .22s cubic-bezier(.16,1,.3,1);">

        <div style="padding:14px 20px;display:flex;align-items:center;gap:10px;background:var(--green-bg);">
            <span style="font-size:20px;">✓</span>
            <span style="font-size:11px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--green);">Confirmation</span>
        </div>
        ...
        <div style="padding:24px 28px 20px;">
            ...
            <p style="font-size:13px;color:var(--muted);font-weight:300;line-height:1.6;">
                Ce message sera marqué comme lu.
            </p>
        </div>

        <div style="padding:0 28px 24px;display:flex;justify-content:flex-end;gap:10px;">
            <button onclick="closeMarkConfirm()"
                style="padding:9px 22px;border-radius:8px;border:1.5px solid var(--border);
                       background:transparent;color:var(--muted);font-family:var(--sans);
                       font-size:12px;font-weight:500;cursor:pointer;letter-spacing:.04em;
                       transition:all .18s;"
            ...
            <button id="confirmMarkBtn"
                style="padding:9px 22px;border-radius:8px;border:none;background:var(--green);
                       font-family:var(--sans);font-size:12px;font-weight:600;
                       letter-spacing:.06em;text-transform:uppercase;cursor:pointer;
                       transition:all .18s;color:#fff;">
                Confirmer
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') { closeDrawer(); closeDeleteConfirm(); closeMarkConfirm(); }
});
...
function openConfirmDelete(messageId, messageName) {
    document.getElementById('deleteMessageName').textContent = messageName;
    
    deleteCallback = function() {
        document.getElementById('form-delete-' + messageId).submit();
    };
...
// ── MODALE DE CONFIRMATION MARQUER COMME LU ───────────────────────────────────
let markCallback = null;

function openConfirmMark(messageId) {
    markCallback = function() {
        document.getElementById('form-mark-' + messageId).submit();
    };
    
    document.getElementById('confirmMarkModal').style.display = 'flex';
    document.body.style.overflow = 'hidden';
}

function closeMarkConfirm() {
    document.getElementById('confirmMarkModal').style.display = 'none';
    document.body.style.overflow = '';
    markCallback = null;
}

document.getElementById('confirmMarkBtn').addEventListener('click', function() {
    if (markCallback) markCallback();
    closeMarkConfirm();
});

document.getElementById('confirmMarkModal').addEventListener('click', function(e) {
    if (e.target === this) closeMarkConfirm();
});
</script>
