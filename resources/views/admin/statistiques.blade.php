...
<div class="drawer" id="drawer">
    ...
    <div class="drawer-body">
        ...
        <a href="{{ route('admin.statistiques') }}" class="drawer-item active">
...
{{-- ════════════════════════════════════════════
     MAIN CONTENT - STATISTIQUES
════════════════════════════════════════════ --}}
<div class="admin-shell">
    <main class="main">

        <div class="page-header">
            <h1>Vue <em>statistiques</em></h1>
            <p>Analyses de lectures, auteurs actifs et tendances de la plateforme.</p>
        </div>
        ...
        <div class="tabs">
            ...
            <a href="{{ route('admin.statistiques') }}" class="tab active">Statistiques</a>
        ...
        <div class="stats-grid">
            <div class="stat-card c-terra">
                ...
                <div class="stat-value">{{ number_format($totalLecteurs) }}</div>
                <div class="stat-sub">tous les articles confondus</div>
            </div>
            <div class="stat-card c-green">
                <div class="stat-label">Auteurs actifs</div>
                <div class="stat-value">{{ $auteursActifs->count() }}</div>
                <div class="stat-sub">avec au moins un article publié</div>
            </div>
            <div class="stat-card c-blue">
                <div class="stat-label">Mois le plus lu</div>
                <div class="stat-value">
                    @php 
                        $topMonth = $viewsPerMonth->sortByDesc('total')->first(); 
                        $monthNames = ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Jun', 'Jul', 'Aoû', 'Sep', 'Oct', 'Nov', 'Déc'];
                    @endphp
                    {{ $topMonth ? $monthNames[$topMonth->month - 1] . ' ' . $topMonth->year : '—' }}
                </div>
                <div class="stat-sub">{{ $topMonth ? number_format($topMonth->total) . ' lectures' : 'Aucune donnée' }}</div>
            </div>
        </div>
        ...
        <div class="charts-grid">
            <div class="chart-card">
                <div class="chart-title">📈 Lectures par mois</div>
                <canvas id="viewsChart" height="200"></canvas>
            </div>
            <div class="chart-card">
                <div class="chart-title">👥 Nouveaux auteurs par mois</div>
                <canvas id="auteursChart" height="200"></canvas>
            </div>
        </div>

        {{-- TABLEAU DES AUTEURS ACTIFS --}}
        <div class="table-panel">
            <div class="panel-header">🏆 Auteurs actifs sur la plateforme</div>
            @if($auteursActifs->isEmpty())
                <div class="empty-state">
                    <div class="empty-state-icon">✦</div>
                    Aucun auteur actif pour le moment.
                </div>
            @else
                <div style="overflow-x: auto;">
                    <table>
                        ...
                        <tbody>
                            @foreach($auteursActifs as $auteur)
                            <tr>
                                <td>
                                    <div class="author-cell">
                                        @if($auteur->avatar)
                                            <img src="{{ asset('img/' . $auteur->avatar) }}" style="width:36px;height:36px;border-radius:10px;object-fit:cover;">
                                        @else
                                            <div class="avatar">{{ strtoupper(substr($auteur->name, 0, 2)) }}</div>
                                        @endif
                                        <span class="author-cell-name">{{ $auteur->name }}</span>
                                    </div>
                                </td>
                                <td style="color:var(--muted);">{{ $auteur->email }}</td>
                                <td><strong style="color:var(--terracotta); font-family:var(--serif); font-size:1.2rem;">{{ $auteur->posts_count }}</strong></td>
...
const viewsData = @json($viewsPerMonth);
const auteursData = @json($auteursParMois);
const monthNames = ['Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Jun', 'Jul', 'Aoû', 'Sep', 'Oct', 'Nov', 'Déc'];

Chart.defaults.font.family = "'Jost', sans-serif";
Chart.defaults.color = '#7a6a58';

// Graphique des lectures par mois
if (document.getElementById('viewsChart')) {
    new Chart(document.getElementById('viewsChart'), {
        type: 'bar',
        data: {
            labels: viewsData.map(d => monthNames[d.month - 1] + ' ' + d.year),
            datasets: [{
                label: 'Lectures',
                data: viewsData.map(d => d.total),
                backgroundColor: '#c0623a',
                borderRadius: 6,
                borderSkipped: false,
            }]
        },
...
// Graphique des nouveaux auteurs par mois
if (document.getElementById('auteursChart')) {
    new Chart(document.getElementById('auteursChart'), {
        type: 'line',
        data: {
            labels: auteursData.map(d => monthNames[d.month - 1] + ' ' + d.year),
            datasets: [{
                label: 'Nouveaux auteurs',
                data: auteursData.map(d => d.total),
                borderColor: '#2a2018',
                backgroundColor: 'rgba(42,32,24,0.07)',
                borderWidth: 2,
                tension: 0.4,
                fill: true,
