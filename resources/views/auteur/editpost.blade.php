...
.field-error { font-size: 11px; color: var(--red); margin-top: 5px; font-weight: 400; }
...
.file-name-display { font-size: 12px; color: var(--muted); margin-top: 6px; font-weight: 300; }
...
.btn-secondary { background: var(--warm-off); color: var(--ink); border-color: var(--border); }
.btn-secondary:hover { background: var(--warm-deep); }
    ...
    <main class="main">
        ...
        <div class="edit-container">
            ...
            <div class="page-header">
                <h1>Modifier <em>l'article</em></h1>
                <p>Mettez à jour le contenu et soumettez à validation.</p>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('auteur.updatepost', $post->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label>Titre</label>
                            <input type="text" name="title" value="{{ old('title', $post->title) }}" placeholder="Titre de votre article…" required>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                <label>Statut</label>
                                <select name="status">
                                    <option value="pending" {{ $post->status === 'pending' ? 'selected' : '' }}>Soumettre à l'admin</option>
                                    <option value="draft"   {{ $post->status === 'draft' ? 'selected' : '' }}>Garder en brouillon</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            @if($post->image)
                                <img src="{{ asset('img/' . $post->image) }}" alt="" class="image-current">
                            @endif
                            ...
                            <p class="file-name-display" id="fileNameDisplay">Aucun fichier sélectionné</p>
                            <p class="field-hint">Laisser vide pour conserver l'image actuelle. PNG, JPG, WEBP jusqu'à 5MB.</p>
...
function updateFileName(input) {
    ...
    display.textContent = input.files.length ? input.files[0].name : 'Aucun fichier sélectionné';
}
